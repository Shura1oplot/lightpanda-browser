...
Дата актуализации: 27 августа 2026 года. Редакция 9.
...
4. Для сборки создается временный платный сервер Timeweb. Один локальный сеанс Bash управляет циклом от создания до проверки удаления, а длительная удаленная работа выполняется в `tmux`.
...
5. Сервер №8932557 перешел в состояние `on`, но первый `ssh-keyscan` на порту 443 получил `Broken pipe`. Обработчик сразу удалил сервер и адрес до передачи исходников. Состояние `on` не означает готовность нестандартного порта SSH. В следующем цикле ожидайте успешный `ssh-keyscan` до десяти минут на том же сервере. Одиночная ошибка подключения не должна завершать разрешенный цикл.

6. Длительные этапы сборки и тестов запускайте в именованном сеансе `tmux`. Оболочка сеанса должна сохранять код завершения и краткую сводку в файлах на сервере. При разрыве SSH подключитесь повторно и проверьте тот же сеанс. Не удаляйте сервер, пока `tmux` активен или результат не известен.

7. Первый запуск в `tmux` остановился до компиляции: `ftp.gnu.org` не ответил на HTTPS-подключение и `curl` завершился с кодом 28. Не загружайте внешние архивы внутри критического длительного этапа. Заранее загрузите все архивы и файлы V8 из официальных источников, сверьте их с ожидаемыми SHA-256, передайте на сервер и повторите сверку до запуска `tmux`. Для `libidn2-2.3.7.tar.gz` используйте официальную точку `https://ftpmirror.gnu.org/libidn/libidn2-2.3.7.tar.gz` и SHA-256 `4c21a791b610b9519b9d0e12b8097bf2f359b12f8dd92647611a929e6bfd7d64`.
...
02. Зафиксируйте SHA Lightpanda и curl-impersonate. До создания ресурса подготовьте `git archive` обоих деревьев, загрузите все внешние архивы и файлы V8, запишите их ожидаемые SHA-256 и выполните локальную сверку.
...
08. Передайте на сервер заранее подготовленные входные файлы и повторно проверьте их SHA-256. Установите `tmux`. Передайте проверенный сценарий и оболочку, запустите curl-impersonate, Lightpanda и тесты в одном именованном сеансе. Получайте состояние через файлы кода завершения и сводки. Сохраните исполняемый файл в `/opt/out` до обращения к внешним сайтам.
...
10. После завершения `tmux` получите исполняемый файл и протокол, сравните удаленную и локальную контрольные суммы, затем удалите сервер и адрес. Завершайте цикл только после повторных запросов API с пустым результатом.
...
10. При потере SSH проверьте состояние того же сеанса `tmux` и файла кода завершения. Пока сеанс активен или результат не известен, не запускайте очистку. При недоступности API или неподтвержденном удалении проверьте ресурсы вручную. Не оставляйте сервер после известного исхода без нового разрешения пользователя.

11. До команды создания сервера локально подготовьте и проверьте все архивы исходного кода, внешних зависимостей и V8. Сетевая ошибка после создания платного ресурса не должна блокировать компиляцию.
...
until ssh-keyscan -p "$ssh_port" -H "$server_ip" \
  >"$known_hosts_file" 2>/dev/null; do
...
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y autoconf automake binutils build-essential bzip2 ca-certificates cmake curl file git golang-go gperf jq libtool make ninja-build patch pkg-config python3 ripgrep tmux unzip xz-utils
...
Архивы и реестр SHA-256 подготовьте локально до `twc server create`. На этом этапе только передайте их на сервер и повторите сверку. До запуска соберите команды этого и следующих разделов в один проверенный Bash-сценарий. Проверьте его локально через `bash -n`, `shellcheck` и `vkus-bash lint`. Передайте сценарий на сервер вместе с оболочкой, которая:

1. Запускает сценарий в именованном сеансе `tmux`.

2. Пишет потоки вывода в `/opt/out/tmux-build.log`.

3. Пишет точный код завершения в `/opt/out/tmux-exit-code` и краткую сводку в `/opt/out/tmux-summary.env`.

После запуска отключитесь от терминала `tmux`. Опрашивайте состояние через новые короткие SSH-подключения. Если связь пропала, не запускайте вторую сборку: повторно проверьте тот же сеанс и файлы результата.

Для фиксаций `1bcebafd4bdedf139520614c8c041df6d36c3926` и `266cd9ffce634930e7b236fe340016718ac29dba` проверен следующий реестр входных файлов:

```text
4c21a791b610b9519b9d0e12b8097bf2f359b12f8dd92647611a929e6bfd7d64  libidn2-2.3.7.tar.gz
...
816c96e8e8f193b40151dad7e8ff37b1221d019dbcb9c35cd3fadbfe6477dfec  v1.2.0.tar.gz
450e169b284697c6eafe523cb7679f4221fbe1a0993f773a566c33b188762844  156c7b75ae9b8c3b3f847acf264f17594c3859fb.zip
607b174554d22a828bc532d1d734fe0f729b5d5ed207f2f12e96a62e83f29c55  nghttp2-1.63.0.tar.bz2
871ec97ad86803cf312901b0c393b0ee70163e25a87c9b2894d1234341ce4e97  ngtcp2-1.20.0.tar.bz2
c6c491a52804814098e446630e6efc459afc0d3da7952ffe6cbdc0b3f99b2b62  nghttp3-1.15.0.tar.bz2
ec753aa6f408a3ca9f0d6d5f7a77417aecd1544db13c03ae5d443612bf367364  curl-8_21_0.tar.gz
c226f71561aa242f278821cebe57c8321e980fd1e491285bde05b70d126f2dac  libc_v8_14.9.207.35_linux_x86_64.a
5023bf13334882f015b363b252516d9e202153aa7350918008ed130ca69b3d30  libc_v8_14.9.207.35_linux_x86_64.so
```
...
14. Длительные этапы выполнены в одном именованном сеансе `tmux`; код завершения и сводка сохранены в файлах на сервере.

15. Все внешние архивы и файлы V8 переданы до сборки и дважды сверены с ожидаемыми SHA-256: локально и на сервере.
