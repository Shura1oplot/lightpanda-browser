...
fn tick(self: *const Driver) !bool {
    // Preserve a delivered WebSocket protocol error before general shutdown.
    ...
    if (self.browser.env.terminatePending()) {
        // The terminal message is published before requestTerminate. Drain again
        // after observing the acquire-load to close the delivery race.
        self.browser.http_client.drainTerminalInbox() catch |err| switch (err) {
            error.ClientDisconnected => return false,
        };
